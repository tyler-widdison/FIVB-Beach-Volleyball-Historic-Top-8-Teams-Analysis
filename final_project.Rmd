...
ggplot(t3)+
  ...
  labs(title = 'Matches played & won', color = '', x = 'Year' , y= 'Matches')+
...
ggplot(c2, aes(court, total_matches)) + 
  ...
  labs(title = 'Win % and matches played per court',
       x = 'Court',
       y = 'Matches')
...
ggplot(h, aes(hour, total_matches)) + 
  ...
  theme_tq() +
  labs(title = 'Win % and matches played per hour',
     x = 'Hour',
     y = 'Matches')
```
...
### **Women**
#### The highest probability for a men's team to win in the top 8 is court 5 at 09:00. With the loweset being at 21:00 on court 1.
```{r, echo = FALSE, error=TRUE}
f2 <- df_women %>%
  dplyr::group_by(hour, court) %>%
  dplyr::summarise(total_matches = sum(total_matches),
